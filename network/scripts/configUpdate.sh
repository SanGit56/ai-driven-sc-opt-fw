...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config ${NETWORK_DIR}/configtx/channel-artifacts/config_block.pb -o 10.125.169.226:7050 --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
